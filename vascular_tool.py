...
def get_global_threshold(images):
    ...
    for i,imageLoc in enumerate(images):
        if i % 10 == 0:
            ...
            imgAdapted = (skimage.exposure.equalize_adapthist(imgGrey,nbins=256)*255).astype(np.uint8)
    ...
    kMeansHisto, _ = kmeans1d.cluster(histoTotal, 2)
    #Get position of changes in kmeans -> cluster
    clusterChange = np.where(np.roll(kMeansHisto,1)!=kMeansHisto)
    return np.max(clusterChange)
...
def import_and_blur_image(imgPath, sigma = 2.5):
    ...
    imgGrey = img[:,:,1] #Take the green channel
    adapted_hist = equalize_adapthist(imgGrey)
...
def main(path: str, savename: str):
    ...
    savename = ".\\08_04_24_test_global_threshold_WELL_C9.csv"
    resultsPath = './Results/'
            ...
            for i, image in enumerate(images):
                if i % 10 ==0:
                    ...
                    draw_and_save_images(rgbimg,
                       segmentation, os.path.abspath(resultsPath + str(i)+'.tiff'))
                bar()
    except Exception as e:
        print(e)
